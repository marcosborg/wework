            ...
            <div class="form-group">
                ...
                @foreach(App\Models\Company::THEME_RADIO as $key => $label)
                    <div class="form-check {{ $errors->has('theme') ? 'is-invalid' : '' }}">
                        <input class="form-check-input" type="radio" id="theme_{{ $key }}" name="theme" value="{{ $key }}" {{ old('theme', $company->theme) === (string) $key ? 'checked' : '' }} required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('funnels') ? 'is-invalid' : '' }}" name="funnels[]" id="funnels" multiple>
                    @foreach($funnels as $id => $funnel)
                        <option value="{{ $id }}" {{ (in_array($id, old('funnels', [])) || $company->funnels->contains($id)) ? 'selected' : '' }}>{{ $funnel }}</option>
                    @endforeach
                </select>
